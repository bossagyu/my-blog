...
![全探索とANNの違い：全探索はすべての点と距離計算するのに対し、ANNは近そうな領域だけを調べる](img-051-001.svg)
...
![HNSWの階層構造：上の層で大まかに近づき、下の層に降りて精密に探索する](img-051-002.svg)
...
![PQの仕組み：ベクトルをサブベクトルに分割し、各部分を代表点のコードに置き換えて512バイトを4バイトに圧縮する](img-051-003.svg)
...
```mermaid
flowchart LR
    A[クエリベクトル] --> B["ANNインデックスで粗く絞り込み<br/>(HNSW / IVF + 量子化)"]
    B -->|"候補 上位100件"| C["リランキング<br/>(元のベクトルで厳密に再計算)"]
    C -->|"上位10件"| D[最終結果]
```
